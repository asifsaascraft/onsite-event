...
const protect = asyncHandler(async (req, res, next) => {
  ...
  if (decoded.role === "operator") {
    if (!decoded.eventId) {
      throw new AppError("Invalid operator access token.", 401);
    }

    const event = await Event.findById(decoded.eventId);

    if (!event) {
      throw new AppError("Event no longer exists.", 401);
    }
    ...
    if (new Date() > event.endDate) {
      throw new AppError(
        "Event has ended. Operator access is no longer available.",
        401,
      );
    }

    // ==========================================
    // Verify Operator Login Code
    // ==========================================

    if (decoded.operatorLoginCode !== event.operatorLoginCode) {
      throw new AppError("Invalid operator access token.", 401);
    }
